...
export function useOfflineMode() {
  ...
  useEffect(() => {
    ...
    if (savedTransactions) {
      try {
        ...
        console.error('Failed to parse offline _transactions:', error);
  ...
  const saveOfflineTransaction = useCallback((_transaction: any) => {
  ...
  const clearSyncedTransactions = useCallback((_syncedIds: string[]) => {
    setOfflineTransactions(prevTransactions => {
  ...
  return {
    ...
    _hasPendingTransactions: offlineTransactions.length > 0
  };
}
