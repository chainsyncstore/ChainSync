...
export default function LoyaltyPage() {
  ...
  const { data: selectedMember, isLoading: memberLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      try {
        const response = await apiRequest(
          'GET',
          `/api/loyalty/member/${selectedMemberId}/activity`
        );
        return await response.json();
      } catch (error) {
              ...
              <Dialog open={enrollDialog} onOpenChange={setEnrollDialog}>
                ...
                <DialogContent>
                  <DialogHeader>
                    ...
                    <DialogDescription>
                      Enter customer details to create an account and enroll them in the loyalty
                      program.
                    </DialogDescription>
                  </DialogHeader>
                              ...
                              <div key={reward.id} className="border rounded-lg p-4">
                                ...
                                <div className="mt-4">
                                  ...
                                  {reward.discountType === 'free_product' && reward.product && (
                                    <div className="text-sm">
                                      Free {reward.product.name} (₦
                                      {formatNumber(reward.product.price)} value)
                                    </div>
                                  )}
                                </div>
                                <div className="mt-2">
                            ...
                            <Table>
                              ...
                              <TableBody>
                                {analytics.topRewards.map(
                                  (
                                    reward: { name: string; redemptions: number },
                                    index: number
                                  ) => (
                                    <TableRow key={index}>
                          ...
                          {analytics.programDetails ? (
                            <div className="space-y-4">
                              ...
                              <div>
                                ...
                                <p>
                                  {analytics.programDetails.pointsPerAmount} points per currency
                                  unit
                                </p>
                              </div>
                              <div>
