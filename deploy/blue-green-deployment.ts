  ...
  async getCurrentActiveEnvironment(): Promise<string> {
    try {
      ...
      try {
        ...
        this.inactiveEnv = this.activeEnv === this.config.blueEnv 
          ? this.config.greenEnv 
          : this.config.blueEnv;
      ...
      throw new Error(`Failed to determine current active environment: ${(error as Error).message}`);
    }
  }
