...
class DeploymentValidator {
  ...
  private printResults(): void {
    ...
    if (this.results.failures.length === 0) {
      ...
      console.log(
        `⛔ Deployment validation FAILED with ${this.results.failures.length} issues that must be fixed.`
      );
    }

    if (this.results.warnings.length > 0) {
      console.log(
        `⚠️ There are ${this.results.warnings.length} warnings that should be addressed.`
      );
    }
  }
}
