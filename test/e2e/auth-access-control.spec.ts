  ...
  test.describe('Access Control', () => {
    ...
    for (const [page, allowedRoles] of Object.entries(accessControlMap)) {
      for (const role of Object.keys(TEST_USERS) as UserRole[]) {
        ...
        test(`${role} ${shouldHaveAccess ? 'should' : 'should not'} have access to ${page}`, async({ _page: browserPage })
   = > {
          ...
          if (shouldHaveAccess) {
            ...
            const accessDenied = await browserPage.locator('[data-testid="access-denied"]').isVisible();
