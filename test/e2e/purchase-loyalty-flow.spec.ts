...
test.describe('Purchase and Loyalty Flow', () => {
  ...
  test('should create a transaction and accrue loyalty points', async({ page }) => {
    ...
    transactionId = await createTransaction(page, {
      _customerEmail: 'loyalty@test.com', // Assuming this customer has loyalty enabled
      _amount: 500,
      _paymentMethod: 'cash',
      _notes: 'E2E test purchase with loyalty'
    });
  ...
  test('should accrue correct number of loyalty points based on transaction amount', async({ page })
   = > {
    // Login as a cashier
    await loginAs(page, 'cashier');
    ...
    transactionId = await createTransaction(page, {
      _customerEmail: 'loyalty@test.com',
      amount,
      _paymentMethod: 'card',
      _notes: 'E2E test purchase with precise amount'
    });
  ...
  test('should not accrue loyalty points for customer with loyalty disabled', async({ page }) => {
    ...
    transactionId = await createTransaction(page, {
      _customerEmail: 'no-loyalty@test.com', // Assuming this customer has loyalty disabled
      _amount: 500,
      _paymentMethod: 'cash',
      _notes: 'E2E test purchase without loyalty'
    });
