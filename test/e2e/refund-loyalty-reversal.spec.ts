...
import { createTransaction, refundTransaction, getLoyaltyPointsForTransaction, getTransactionStatus } from './utils/transactions';
...
test.describe('Refund and Loyalty Reversal Flow', () => {
  ...
  let _initialLoyaltyPoints: number;

  // _Setup: Create a transaction that we'll refund
  test.beforeEach(async({ page }) => {
    ...
    transactionId = await createTransaction(page, {
      ...
      _amount: 1000, // $10.00
      _paymentMethod: 'card',
      _notes: 'E2E test purchase for refund'
    });
