...
class LoyaltyService {
  ...
  async reverseLoyaltyPoints(customerId: number, amountRefunded: number): Promise<void> {
    ...
    const customer =
      await this./* REPLACE_PRISMA: Replace this logic with Drizzle ORM or test double */ customer.findUnique(
        { where: { id: customerId } }
      );
...
describe.integration('LoyaltyService Reversal', () => {
  ...
  test.integration('should reverse loyalty points on refund', async () => {
    const customer =
      await /* REPLACE_PRISMA: Replace this logic with Drizzle ORM or test double */ customer.create(
        { data: makeMockCustomer({ loyaltyPoints: 15 }) }
      );
  ...
  test.integration('should not allow loyaltyPoints to go negative', async () => {
    const customer =
      await /* REPLACE_PRISMA: Replace this logic with Drizzle ORM or test double */ customer.create(
        { data: makeMockCustomer({ loyaltyPoints: 3 }) }
      );
