...
export const storage = {
  ...
  async createInventory(data: schema.InventoryInsert) {
    const [inventory] = await db.insert(schema.inventory)
      .values(data)
      .returning();
    return inventory;
  },
  ...
  async createInventoryBatch(data: schema.InventoryBatchInsert) {
    const [batch] = await db.insert(schema.inventoryBatches)
      .values(data)
      .returning();
  ...
  async updateInventoryBatch(batchId: number, data: Partial<schema.InventoryBatchInsert>) {
    ...
    const [updated] = await db.update(schema.inventoryBatches)
      .set({
        ...data,
        updatedAt: new Date()
      })
    ...
    try {
      ...
      for (const item of items) {
        try {
          ...
          const [transItem] = await db.insert(schema.transactionItems)
            .values({
              ...item,
  ...
  async getReturnAnalytics(storeId?: number, startDate?: Date, endDate?: Date) {
    ...
    const totalRefundResult = await db.select({
    ...
    const totalReturnsResult = await db.select({
      count: count()
    })
    .from(schema.refunds)
    .where(whereClause);
    ...
    const reasonsQuery = await db.select({
      reasonId: schema.refundItems.returnReasonId,
      count: count(),
    })
    .from(schema.refundItems)
    .leftJoin(schema.refunds, eq(schema.refundItems.refundId, schema.refunds.id))
    .where(whereClause)
    ...
    const reasonsMap = returnReasons.reduce((acc, reason) => {
    ...
    const restockedQuery = await db.select({
