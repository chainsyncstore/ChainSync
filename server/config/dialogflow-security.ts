...
export function verifyDialogflowConfig(): boolean {
  ...
  if (!googleCredentialsPath) {
    log(
      'Warning: GOOGLE_APPLICATION_CREDENTIALS environment variable not set. Dialogflow functionality will be limited.'
    );
    return false;
  }
  ...
  if (!projectId) {
    log(
      'Warning: DIALOGFLOW_PROJECT_ID environment variable not set. Dialogflow functionality will be limited.'
    );
    return false;
  }
 ...
 */
export function enforceHttpsForDialogflowRoutes(
  req: express.Request,
  ...
  const isDialogflowRoute =
    req.path.includes('/api/ai/') || req.path.includes('/webhooks/dialogflow');
