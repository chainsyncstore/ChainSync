...
describe('TransactionService', () => {
  ...
  beforeEach(() => {
    ...
    mockInventoryService.adjustInventory = jest
      .fn<
        (
    ...
    mockLoyaltyService.awardPoints = jest
      .fn<
        (
  ...
  describe('createTransaction', () => {
    ...
    it('should create a transaction with validated data', async () => {
      ...
      dbMock
        .insert()
        .values()
        .returning.mockResolvedValueOnce([{ id: 1, ...validTransactionData }]);
    ...
    it('should throw error when store does not exist', async () => {
      ...
      const storesFindFirstMock = dbMock.query.stores.findFirst;
      ...
      await expect(transactionService.createTransaction(validTransactionData)).rejects.toThrow(
        TransactionServiceErrors.STORE_NOT_FOUND.message
      );
    });
    ...
    it('should throw error when user does not exist', async () => {
      ...
      dbMock.query.stores.findFirst.mockResolvedValue({ id: 1, name: 'Test Store' });
      const usersFindFirstMock = dbMock.query.users.findFirst;
      usersFindFirstMock.mockResolvedValue(null);

      await expect(transactionService.createTransaction(validTransactionData)).rejects.toThrow(
        TransactionServiceErrors.USER_NOT_FOUND.message
      );
    });
    ...
    it('should throw error when customer does not exist', async () => {
      ...
      dbMock.query.stores.findFirst.mockResolvedValue({ id: 1, name: 'Test Store' });
      dbMock.query.users.findFirst.mockResolvedValue({ id: 1, name: 'Test User' });
      const customersFindFirstMock = dbMock.query.customers.findFirst;
      customersFindFirstMock.mockResolvedValue(null);

      await expect(transactionService.createTransaction(validTransactionData)).rejects.toThrow(
        TransactionServiceErrors.CUSTOMER_NOT_FOUND.message
      );
    });
    ...
    it('should throw error when product not found', async () => {
      ...
      await expect(transactionService.createTransaction(validTransactionData)).rejects.toThrow(
        TransactionServiceErrors.PRODUCT_NOT_FOUND.message
      );
    });
    ...
    it('should throw error when insufficient stock', async () => {
      ...
      const productsFindManyMock = dbMock.query.products.findMany;
      ...
      await expect(transactionService.createTransaction(validTransactionData)).rejects.toThrow(
        TransactionServiceErrors.INSUFFICIENT_STOCK.message
      );
    });
  ...
  describe('processRefund', () => {
    ...
    const mockTransaction = {
      ...
      items: [{ id: 1, productId: 1, quantity: 2, unitPrice: '50.00' }],
    };
    ...
    it('should process a full refund successfully', async () => {
      ...
      dbMock
        .insert()
        .values()
        .returning.mockResolvedValueOnce([{ id: 1, refundId: 'REF-123' }]);

      const result = await transactionService.processRefund(validRefundParams);

      // Check that validation was called
      expect(
        require('@shared/schema-validation').transactionValidation.refund.insert
      ).toHaveBeenCalled();
    ...
    it('should throw error when transaction not found', async () => {
      ...
      await expect(transactionService.processRefund(validRefundParams)).rejects.toThrow(
        TransactionServiceErrors.TRANSACTION_NOT_FOUND.message
      );
    });
    ...
    it('should throw error when transaction already refunded', async () => {
      ...
      await expect(transactionService.processRefund(validRefundParams)).rejects.toThrow(
        TransactionServiceErrors.INVALID_REFUND.message
      );
    });
    ...
    it('should handle partial refunds', async () => {
      ...
      const partialRefundParams = {
        ...validRefundParams,
        fullRefund: false,
        items: [{ transactionItemId: 1, quantity: 1, unitPrice: '50.00', isRestocked: true }],
      };
  ...
  describe('getTransactionAnalytics', () => {
    ...
    it('should throw error when store not found', async () => {
      ...
      await expect(transactionService.getTransactionAnalytics(1)).rejects.toThrow(
        TransactionServiceErrors.STORE_NOT_FOUND.message
      );
    });
  });
