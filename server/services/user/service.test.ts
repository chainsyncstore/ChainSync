...
jest.mock('bcrypt', () => ({
  ...
  compare: jest.fn(async (data: string | Buffer, encrypted: string): Promise<boolean> => true),
}));
...
describe('UserService', () => {
  ...
  const mockLogger = {
    ...
    fatal: jest.fn(), // Added fatal based on typical logger interface
    trace: jest.fn(), // Added trace based on typical logger interface
    silent: jest.fn(), // Added silent based on typical logger interface
    getPino: jest.fn(), // Added getPino based on typical logger interface
    level: 'info', // Added level based on typical logger interface
    isLevelEnabled: jest.fn().mockReturnValue(true), // Added isLevelEnabled
  };

  beforeEach(() => {
    userService = new UserService({ logger: mockLogger, serviceName: 'UserServiceTest' });
    jest.clearAllMocks();
  });
  ...
  describe('createUser', () => {
    ...
    it('should create a new user with validated data', async () => {
      ...
      (db.insert().values().returning as jest.MockedFunction<any>).mockResolvedValue([
    ...
    it('should throw error when username already exists', async () => {
      ...
      await expect(userService.createUser(validUserData)).rejects.toThrow(
        UserServiceErrors.DUPLICATE_USERNAME.message
      );
    });

    it('should throw error when email already exists', async () => {
      // Mock findFirst to return null for username check, but an existing user for email check
      db.query.users.findFirst.mockResolvedValueOnce(null).mockResolvedValueOnce({
        id: 1,
        email: validUserData.email,
      });

      await expect(userService.createUser(validUserData)).rejects.toThrow(
        UserServiceErrors.DUPLICATE_EMAIL.message
      );
    });
  ...
  describe('validateCredentials', () => {
    ...
    it('should throw error for invalid username', async () => {
      ...
      await expect(userService.validateCredentials('wronguser', 'Test@123')).rejects.toThrow(
        UserServiceErrors.INVALID_CREDENTIALS.message
      );
    });
    ...
    it('should throw error for incorrect password', async () => {
      ...
      (bcrypt.compare as jest.MockedFunction<typeof bcrypt.compare>).mockResolvedValueOnce(false);

      await expect(userService.validateCredentials('testuser', 'WrongPassword')).rejects.toThrow(
        UserServiceErrors.INVALID_CREDENTIALS.message
      );
    });
  });
