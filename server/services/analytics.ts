 ...
 */
export async function getStorePerformanceComparison(startDate?: Date, endDate?: Date) {
    ...
    stores.map(async store => {
      try {
        ...
        const topProducts = await db
          ...
          .where(sql`${schema.transactions.storeId} = ${store.id} AND ${transactionWhereClause}`)
  ...
  const globalAvgTransaction =
    globalTotal.transactionCount > 0 ? globalTotal.totalRevenue / globalTotal.transactionCount : 0;
