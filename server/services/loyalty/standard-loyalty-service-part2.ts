...
export class StandardLoyaltyServicePart2 extends BaseService<
  LoyaltyProgram,
  z.infer<typeof programCreateSchema>,
  z.infer<typeof programUpdateSchema>
> {
  ...
  protected readonly createSchema = programCreateSchema as unknown as z.ZodType<{
    storeId: number;
    name: string;
    isActive: boolean;
  ...
  // Cast the schema to match the expected types in BaseService
  protected readonly updateSchema = programUpdateSchema as unknown as z.ZodType<{
    storeId?: number;
    name?: string;
    isActive?: boolean;
    description?: string;
  ...
  async updateProgram(
    id: number,
    data: z.infer<typeof programUpdateSchema>
  ...
  async getProgramsByStoreId(storeId: number): Promise<LoyaltyProgram[]> {
    try {
      ...
      const programs = await this.executeQuery(async db => {
        return db.select().from(loyaltyPrograms).where(eq(loyaltyPrograms.storeId, storeId));
  ...
  async createReward(data: z.infer<typeof rewardCreateSchema>): Promise<LoyaltyReward> {
    try {
      ...
      if (!reward) {
        throw new ServiceError(ErrorCode.DATABASE_ERROR, 'Failed to create loyalty reward', {
          data: validatedData,
        });
      }
  ...
  async getRewardsByProgramId(programId: number): Promise<LoyaltyReward[]> {
    try {
      ...
      // Fetch from database
      const rewards = await this.executeQuery(async db => {
        return db
          .select()
          .from(loyaltyRewards)
    ...
    try {
      ...
      return await this.withTransaction(async trx => {
        ...
        switch (validatedData.type) {
          ...
          default:
            throw new ServiceError(ErrorCode.VALIDATION_ERROR, 'Invalid transaction type', {
        ...
        if (!updatedMember) {
          throw new ServiceError(ErrorCode.DATABASE_ERROR, 'Failed to update member points', {
            memberId: validatedData.memberId,
          });
        }
    ...
    try {
      ...
      if (this.cache) {
        await this.cache.set(
          cacheKey,
          transactions,
          StandardLoyaltyServicePart2.CACHE_TTL.TRANSACTION
        );
      }
      ...
      return this.handleError(
        error,
        `Error fetching loyalty transactions for member ID: ${memberId}`
      );
    }
  }
  ...
  private async checkAndUpdateTier(trx: any, member: LoyaltyMember): Promise<void> {
    ...
    try {
      ...
      const tiers = await trx
        .select()
        .from(loyaltyTiers)
        .where(and(eq(loyaltyTiers.programId, member.programId), eq(loyaltyTiers.status, 'active')))
  ...
  async generateMemberActivityReport(memberId: number): Promise<{
    ...
    try {
      ...
      if (!memberDetails || !memberDetails.programId) {
        // programId should now be part of LoyaltyMember type
        throw new ServiceError(
          ErrorCode.NOT_FOUND,
      ...
      const programResult = await this.executeQuery(async db => {
        return db
          .select()
          .from(loyaltyPrograms)
      ...
      if (!programForReport) {
        throw new ServiceError(
          ErrorCode.NOT_FOUND,
          `Loyalty program not found for member ${memberId}`
        );
      }
