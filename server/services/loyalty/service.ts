...
// Import shared logger utilities
...
interface ServiceConfig {
  db?: typeof db;
  logger?: Logger; // Use shared Logger type
}
...
export class LoyaltyService {
  ...
  private db = db;
  private cache: any; // Placeholder for cache, can be replaced with actual cache service
  ...
  constructor(config: ServiceConfig = {}) {
    if (config.logger) {
      ...
      this.logger = getLogger('LoyaltyService', { component: 'LoyaltyService' });
    }
  }
   ...
   */
  async enrollCustomer(customerId: number, storeId: number, userId: number): Promise<any> {
    try {
  ...
  async checkAndUpdateMemberTier(memberId: number): Promise<boolean> {
    try {
      ...
      const currentPoints = parseInt(this.safeToString(member.currentPoints) || '0');
   ...
   */
  async createLoyaltyTier(tierData: Partial<LoyaltyTier>): Promise<any> {
    try {
      const now = new Date();
      ...
      const tier = await db
        ...
        VALUES 
        (${this.safeToString(tierData.programId)}, ${this.safeToString(tierData.name)}, ${this.safeToString(tierData.description || '')}, 
        ${this.safeToString(tierData.points_required)}, ${tierData.active === false ? false : true}, ${now}) 
        RETURNING *`
        )
        .then(result => result.rows[0]);
      return tier;
   ...
   */
  async createLoyaltyReward(rewardData: Partial<LoyaltyReward>): Promise<any> {
    try {
      const now = new Date();
    ...
    try {
      ...
      for (const transaction of expiredTransactions) {
        ...
        const currentPoints = Math.max(
          0,
          parseInt(this.safeToString(member.currentPoints) || '0') - points
        );
  ...
  async getLoyaltyStatistics(storeId: number): Promise<{
    ...
    try {
      ...
      const members = await db
        .execute(sql`SELECT * FROM loyalty_members WHERE store_id = ${this.safeToString(storeId)}`)
        .then(result => (result.rows || []) as unknown as LoyaltyMember[]);
      ...
      const program = await db
        ...
        .then(result => (result.rows?.[0] as unknown as LoyaltyProgram) || null);
      ...
      const redemptions = await db
        ...
        .then(result => (result.rows || []) as { name: string; count: string }[]);

      const topRewards = redemptions.map(r => ({
        name: r.name,
   ...
   */
  async redeemReward(memberId: number, rewardId: number, userId: number): Promise<any> {
    try {
      ...
      const member = await db
        ...
        .then(result => (result.rows?.[0] as unknown as LoyaltyMember) || null);
      ...
      const reward = await db
        ...
        .then(result => (result.rows?.[0] as unknown as LoyaltyReward) || null);
      ...
      const totalRedeemed =
        parseInt(this.safeToString(member.totalPointsRedeemed) || '0') + pointsRequired;
  ...
  /**
   * Execute a raw SQL query (helper for future extensibility)
   */
  protected async executeQuery<T = any>(query: SQL): Promise<T[]> {
    try {
      const result = await this.db.execute(query);
      return result.rows as T[];
    } catch (error: unknown) {
      return this.handleError(error, 'Executing query');
    }
  }
   ...
   */
  protected async withTransaction<T>(fn: (trx: typeof db) => Promise<T>): Promise<T> {
    try {
      // If your db supports transactions, use it here
      // Replace with actual transaction logic if needed
      return await (this.db.transaction ? this.db.transaction(fn) : fn(this.db));
    } catch (error: unknown) {
      return this.handleError(error, 'Running transaction');
    }
  }
}
