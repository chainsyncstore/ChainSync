...
export async function enhanceValidationWithAI(
  ...
  try {
    ...
    const responsePreview = responseText.length > 150 
      ? responseText.substring(0, 147) + '...' 
      : responseText;
    ...
    const fixedErrorCount = result.errors.length > 0 ? 
      result.totalRows - result.errors.length : 0;
    const fixedMissingFieldCount = result.missingFields.length > 0 ? 
      result.totalRows - result.missingFields.length : 0;
...
function applyFix(
  ...
  if (result.mappedData[rowIndex]) {
    ...
    const relatedError = result.errors.find(err => 
      err.row - 1 === rowIndex && err.value === oldValue
    );
