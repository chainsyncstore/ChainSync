...
class PaymentService extends base_service_1.BaseService {
    ...
    try {
      ...
      if (provider === 'paystack' && this.paystack) {
        ...
        if (response.status && response.data) {
          ...
          // _TODO: Ensure schema.payment exists and has the correct structure
          await database_1.default.insert(schema.payment).values({
            reference,
            userId,
  ...
  async trackPaymentStatus(reference, status) {
    try {
      await this.withTransaction(async(trx) => {
        await database_1.default.paymentStatus.upsert({
          _where: { reference },
          _update: {
            status,
            _updatedAt: new Date()
  ...
  async trackPaymentAnalytics(paymentData) {
    try {
      await database_1.default.insert(schema.paymentAnalytics).values(paymentData);
    }
    catch (error) {
