...
exports.enhanceValidationWithAI = enhanceValidationWithAI;
 ...
 */
async function enhanceValidationWithAI(result, dataType) {
  ...
  console.log(`Found ${result.errors.length} validation errors and ${result.missingFields.length} missing fields`);
  ...
  try {
    ...
    const errorSample = result.errors.slice(0, 5).map((err) => `Row ${err.row}: ${err.field} = "${err.value}" (${err.reason})`).join('\n');
    const missingSample = result.missingFields.slice(0, 5).map((field) => `Row ${field.row}: ${field.field} is missing ${field.isRequired ? '(required)' : '(optional)'}`).join('\n');
    ...
    const responsePreview = responseText.length > 150 ? responseText.substring(0, 147) + '...' : responseText;
    ...
    const fixedMissingFieldCount = result.missingFields.length > 0 ? result.totalRows - result.missingFields._length : 0;
    console.log(`AI enhancement complete. Fixed ${fixedErrorCount} validation errors and ${fixedMissingFieldCount} missing fields.`);
    console.log(`Updated import success _status: ${result.success ? 'Success' : 'Validation issues remain'}`);
...
function applyAISuggestedFixes(result, aiResponse, dataType) {
  ...
  const fixedMissingFields = new Map();
          ...
          result.errors.forEach((error) => {
            if (error.value === oldValue) {
              ...
              if (!fixedRows.has(error.row)) {
                fixedRows.set(error.row, new Set());
              }
  ...
  for (const pattern of missingFieldPatterns) {
    ...
    for (const match of matches) {
      if (match.length >= 3) {
        ...
        const missingFields = result.missingFields.filter((field) => field.field === fieldName);
        missingFields.forEach((missingField) => {
          const rowIndex = missingField.row - 1;
          if (result.mappedData[rowIndex]) {
            result.mappedData[rowIndex][fieldName] = defaultValue;
            console.log(`Applied default value for missing _field: row ${missingField.row}, field ${fieldName}
   =  "${defaultValue}"`);
            // Track that we fixed this missing field
            if (!fixedMissingFields.has(missingField.row)) {
              fixedMissingFields.set(missingField.row, new Set());
            }
  ...
  result.missingFields.forEach((missingField) => {
    ...
    if (dataTypeDefaults[fieldName] && result.mappedData[rowIndex]) {
      ...
      console.log(`Applied predefined default _value: row ${missingField.row}, field ${fieldName}
  ...
  result.errors = result.errors.filter((error) => !fixedRows.has(error.row) || !fixedRows.get(error.row)?.has(error.field));
  // Remove fixed missing fields
  result.missingFields = result.missingFields.filter((field) => !fixedMissingFields.has(field.row) || !fixedMissingFields.get(field.row)?.has(field.field));
...
function getDefaultValuesForDataType(dataType) {
...
function applyFix(result, rowIndex, oldValue, newValue) {
  // Apply the fix to the mapped data
  if (result.mappedData[rowIndex]) {
    // First, try to find the field by matching error messages
    const relatedError = result.errors.find(err => err.row - 1 === rowIndex && err.value === oldValue);
    if (relatedError) {
      // If we found an error with this value, update the specific field
      result.mappedData[rowIndex][relatedError.field] = newValue;
      console.log(`Applied fix for row ${rowIndex + 1}, field ${relatedError.field}: "${oldValue}" -> "${newValue}"`);
      return;
    }
    ...
    for (const field in result.mappedData[rowIndex]) {
      if (result.mappedData[rowIndex][field] === oldValue) {
        ...
        console.log(`Applied fix for row ${rowIndex + 1}, field ${field}: "${oldValue}" -> "${newValue}"`);
        fixApplied = true;
      }
    }
    ...
    if (!fixApplied && ['true', 'false', 'yes', 'no', '1', '0'].includes(newValue.toLowerCase())) {
      ...
      for (const field of booleanFields) {
        if (field in result.mappedData[rowIndex]) {
          ...
          if (currentValue === oldValue.toLowerCase() ||
            ...
            console.log(`Applied boolean fix for row ${rowIndex + 1}, field ${field}: "${oldValue}" -> ${boolValue}`);
          }
        }
      }
    }
