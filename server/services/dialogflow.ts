...
try {
  ...
  if (process.env.GOOGLE_APPLICATION_CREDENTIALS && process.env.DIALOGFLOW_PROJECT_ID) {
    ...
    if (!process.env.GOOGLE_APPLICATION_CREDENTIALS) {
      console.log(
        'GOOGLE_APPLICATION_CREDENTIALS not found. Dialogflow client will use mock responses.'
      );
    }
...
export async function getDialogflowResponse(userId: number, userMessage: string): Promise<string> {
  try {
    ...
    const responseText =
      response.queryResult.fulfillmentText || "I'm sorry, I couldn't understand your request.";
