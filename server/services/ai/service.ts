...
export class AIService extends BaseService implements AIService {
  ...
  // Text Generation
  async generateCompletion(userId: string, prompt: string, options: unknown = {}): Promise<string> {
    try {
  ...
  async generateChat(userId: string, messages: unknown[], options: unknown = {}): Promise<string> {
    try {
  ...
  // Document Processing
  async generateSummary(userId: string, text: string, options: unknown = {}): Promise<string> {
    try {
      const systemPrompt =
        'You are a helpful assistant. Generate a concise summary of the following text.';
      return await this.generateChat(
        userId,
        [
          { role: 'system', content: systemPrompt },
