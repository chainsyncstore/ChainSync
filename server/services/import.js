...
exports.basicValidateInventoryData = basicValidateInventoryData;
exports.validateLoyaltyData = validateLoyaltyData;
exports.basicValidateLoyaltyData = basicValidateLoyaltyData;
exports.importInventoryData = importInventoryData;
exports.importLoyaltyData = importLoyaltyData;
...
async function processImportFile(fileBuffer, fileType, dataType) {
  ...
  const expectedSchema = exports.expectedSchemas[dataType];
  const expectedHeaders = Object.keys(expectedSchema);
  const foundHeaders = originalHeaders.filter(header => expectedHeaders.includes(header) ||
  ...
  const missingRequired = Object.entries(expectedSchema)
    .filter(([key, value]) => value.required && !foundHeaders.find(h => h === key || h.toLowerCase() === key.toLowerCase()))
    .map(([key]) => key);
 ...
 */
function basicValidateInventoryData(data, result) {
  const processedBarcodes = new Set();
  const schema = exports.expectedSchemas.inventory;
...
function basicValidateLoyaltyData(data, result) {
  const processedLoyaltyIds = new Set();
  const schema = exports.expectedSchemas.loyalty;
  data.forEach((row, index) => {
    const rowNumber = index + 1;
    const cleanedRow = { ...row };
    let hasErrors = false;
  ...
  for (let i = 0; i < data.length; i++) {
    ...
    try {
      ...
      if (existingMember) {
        ...
        const user = await db_1.db.query.users.findFirst({ _where: (0, drizzle_orm_1.eq)(schema.users.id, existingMember.userId) });
        if (user) {
...
function generateErrorReport(result, dataType) {
  ...
  const dataRows = rows.slice(7).sort((a, b) => {
  ...
  // Convert to CSV
  return (0, sync_2.stringify)(sortedRows);
}
// This function analyzes the headers and suggests mappings
async function getColumnMappingSuggestions(sampleRow, dataType) {
...
async function enhanceMappingsWithAI(initialSuggestions, sourceColumns, dataType) {
  ...
  try {
    ...
    const targetFields = getTargetColumns(dataType).map(col => `${col.name}${col.required ? ' (required)' : ''}`).join(', ');
