...
import { IProductService, ProductServiceErrors, CreateProductParams, UpdateProductParams, ProductSearchParams } from './types';
import { db } from '@db';
...
export class ProductService extends BaseService implements IProductService {
  ...
  async searchProducts(params: ProductSearchParams): Promise<{
    ...
    try {
      ...
      if (params.categoryId) {
        whereClause = and(
          whereClause,
          eq(schema.products.categoryId, params.categoryId)
        );
      }
      ...
      if (params.brandId) {
        whereClause = and(
          whereClause,
          eq(schema.products.brandId, params.brandId)
        );
      }
      ...
      if (params.isActive !== undefined) {
        whereClause = and(
          whereClause,
          eq(schema.products.isActive, params.isActive)
        );
      }
      ...
      if (params.minPrice) {
        whereClause = and(
          whereClause,
          gte(schema.products.price, params.minPrice)
        );
      }
      ...
      if (params.maxPrice) {
        whereClause = and(
          whereClause,
          lte(schema.products.price, params.maxPrice)
        );
      }
  ...
  async updateProductInventory(productId: number, quantity: number, reason: string): Promise<boolean> {
    try {
      ...
      await db.insert(schema.inventoryLogs)
        .values({
          productId,
          quantity,
