...
export class EnhancedProductService extends EnhancedBaseService implements IProductService {
  async createProduct(params: CreateProductParams): Promise<SelectProduct> {
    try {
  ...
  async updateProduct(productId: number, params: UpdateProductParams): Promise<SelectProduct> {
    try {
  ...
  async getProductById(productId: number): Promise<SelectProduct | null> {
    try {
  ...
  async getProductBySku(sku: string, storeId: number): Promise<SelectProduct | null> {
    try {
      return await db.query.products.findFirst({
  ...
  async getProductByBarcode(barcode: string, storeId: number): Promise<SelectProduct | null> {
    try {
      return await db.query.products.findFirst({
  ...
  async searchProducts(params: ProductSearchParams): Promise<{ products: SelectProduct[]; total: number; page: number; limit: number; }> {
    try {
  ...
  async getProductsWithLowStock(storeId: number, limit: number = 10): Promise<SelectProduct[]> {
    try {
      ...
      return lowStockProducts.map((p: { products: SelectProduct; inventory: schema.SelectInventory | null; }) => p.products);
    } catch (error) {
