...
const PaystackCustomerMetadataSchema = z.object({
  user_id: z.preprocess((val) => val ? parseInt(String(val), 10) : undefined, z.number().optional()), // Ensure user_id is number
...
const PaystackChargeSuccessDataSchema = z.object({
  ...
  amount: z.preprocess((val) => typeof val === 'string' ? parseInt(String(val), 10) : val, z.number()), // Amount in kobo
...
const FlutterwaveChargeCompletedDataSchema = z.object({
  ...
  amount: z.preprocess((val) => typeof val === 'string' ? parseFloat(val) : val, z.number()),
  currency: z.string(),
  charged_amount: z.preprocess((val) => typeof val === 'string' ? parseFloat(val) : val, z.number()),
  customer: FlutterwaveCustomerSchema,
...
async function handlePaystackSubscriptionCreate(rawData: unknown): Promise<boolean> {
  try {
    ...
    if (!validationResult.success) {
      console.error("Invalid Paystack subscription.create payload:", validationResult.error.flatten().fieldErrors);
      return false;
    }
    const data = validationResult.data;

    const userId = data.customer.metadata?.user_id;
    if (typeof userId !== 'number') { // Check if userId is a number after Zod parsing
      console.error("User ID is missing or not a number in Paystack subscription.create payload's customer metadata");
      return false;
    }
    ...
    if (referralCode) {
      const discountResult = await applyReferralDiscount(userId, referralCode, amountInNaira, currency); // userId is number
    ...
    const subscriptionData: NewSubscription = {
      userId: userId, // userId is now confirmed number
      plan,
      status: "active",
...
async function handlePaystackChargeSuccess(rawData: unknown): Promise<boolean> {
  try {
    const PaystackChargeSuccessDataSchemaWithAmountFix = PaystackChargeSuccessDataSchema.extend({
        amount: z.preprocess((val) => typeof val === 'string' ? parseInt(String(val), 10) : val, z.number()), // Amount in kobo, allow string input
    });
    const validationResult = PaystackChargeSuccessDataSchemaWithAmountFix.safeParse(rawData);
    if (!validationResult.success) {
      console.error("Invalid Paystack charge.success payload:", validationResult.error.flatten().fieldErrors);
      return false;
    }
    const data = validationResult.data;
    ...
    if (typeof userId !== 'number' && !data.subscription?.subscription_code) {
        console.warn("User ID not found or not a number in Paystack charge.success payload, and not a clear subscription renewal.");
        return true; 
    }
    
    const subscriptionCode = data.subscription?.subscription_code || (metadata as any)?.subscription_code; 
    if (subscriptionCode) {
      ...
      if (subscription) {
        ...
        const planInterval = (data.plan && typeof data.plan !== 'string' && 'interval' in data.plan ? data.plan.interval : (metadata as any)?.plan_interval) || 'monthly';
        ...
        console.warn(`Subscription not found for charge.success with reference: ${data.reference} and subscription code: ${subscriptionCode}`);
      }
    } else if (typeof userId === 'number' && (metadata as any).is_subscription_creation) {
        console.log(`Charge success for user ${userId}, potentially a new subscription or one-time payment.`);
    }
...
async function handlePaystackSubscriptionDisable(rawData: unknown): Promise<boolean> {
  try {
    ...
    if (!validationResult.success) {
      console.error("Invalid Paystack subscription.disable payload:", validationResult.error.flatten().fieldErrors);
      return false;
    }
    const data = validationResult.data;
    const [subscription] = await db.select()
    ...
    if (subscription) {
      await db.update(schema.subscriptions)
        ...
        console.warn(`Subscription not found for disable event with subscription_code: ${data.subscription_code}`);
    }
...
async function handleFlutterwaveSubscriptionCreate(rawData: unknown): Promise<boolean> {
  try {
    ...
    if (!validationResult.success) {
      console.error("Invalid Flutterwave subscription.create payload:", validationResult.error.flatten().fieldErrors);
      return false;
    }
    const data = validationResult.data;
    ...
    if (typeof userId !== 'number') {
      console.error("User ID is missing or not a number in Flutterwave subscription.create payload");
      return false;
    }
    const plan = data.plan?.name?.toLowerCase();
...
async function handleFlutterwaveChargeCompleted(rawData: unknown): Promise<boolean> {
  try {
    ...
    if (!validationResult.success) {
      console.error("Invalid Flutterwave charge.completed payload:", validationResult.error.flatten().fieldErrors);
      return false;
    }
    const data = validationResult.data;
    // Extract metadata
...
async function handleFlutterwaveSubscriptionCancelled(rawData: unknown): Promise<boolean> {
  try {
    ...
    if (!validationResult.success) {
      console.error("Invalid Flutterwave subscription.cancelled payload:", validationResult.error.flatten().fieldErrors);
      return false;
    }
    const data = validationResult.data;
    // Find the subscription
