...
describe('InventoryService', () => {
  ...
  describe('createInventory', () => {
    ...
    it('should throw error when product does not exist', async () => {
      ...
      await expect(inventoryService.createInventory(validInventoryData)).rejects.toThrow(
        InventoryServiceErrors.PRODUCT_NOT_FOUND.message
      );
    });
    ...
    it('should throw error when store does not exist', async () => {
      ...
      await expect(inventoryService.createInventory(validInventoryData)).rejects.toThrow(
        InventoryServiceErrors.STORE_NOT_FOUND.message
      );
    });
  ...
  describe('adjustInventory', () => {
    ...
    it('should adjust inventory quantities successfully', async () => {
      ...
      // Check that validation was called
      expect(
        require('@shared/schema-validation').inventoryValidation.adjustment
      ).toHaveBeenCalled();
    ...
    it('should throw error when inventory not found', async () => {
      ...
      await expect(inventoryService.adjustInventory(validAdjustmentData)).rejects.toThrow(
        InventoryServiceErrors.INVENTORY_NOT_FOUND.message
      );
    });
    ...
    it('should throw error when insufficient stock for negative adjustment', async () => {
      ...
      await expect(inventoryService.adjustInventory(negativeAdjustment)).rejects.toThrow(
        InventoryServiceErrors.INSUFFICIENT_STOCK.message
      );
    });
  ...
  describe('addInventoryBatch', () => {
    ...
    it('should add a new batch and adjust inventory accordingly', async () => {
      ...
      // Check that validation was called
      expect(
        require('@shared/schema-validation').inventoryValidation.batch.insert
      ).toHaveBeenCalled();
    ...
    it('should enable batch tracking if not already enabled', async () => {
      ...
      jest
        ...
        .mockResolvedValue({ ...mockInventory, batchTracking: true } as any);
      ...
      expect(inventoryService.updateInventory).toHaveBeenCalledWith(mockInventory.id, {
        batchTracking: true,
      });
    });
    ...
    it('should throw error when product does not exist', async () => {
      ...
      await expect(inventoryService.addInventoryBatch(validBatchData)).rejects.toThrow(
        InventoryServiceErrors.PRODUCT_NOT_FOUND.message
      );
    });
  });
