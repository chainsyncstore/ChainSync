  ...
  async validateData(
    ...
    try {
      ...
      return {
        ...
        errors: result.invalidRecords.map((record, index) => ({
          record,
          errors: result.invalidRecords[index].errors,
        })),
  ...
  async validateFile(file: Express.Multer.File): Promise<{ type: string; data: unknown[] }> {
    try {
      ...
      if (
        ...
        !allowedMimeTypes[extension].includes(file.mimetype)
      ) {
        throw new AppError(
