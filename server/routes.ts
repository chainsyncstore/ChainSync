...
} from './middleware/rate-limiter'; // Updated import
...
export async function registerRoutes(app: Application): Promise<Server> {
  ...
  app.post(
    '/upload',
    fileUploadInstance.uploadFile.bind(fileUploadInstance) as express.RequestHandler
  );
  app.get(
    '/upload/progress/:id',
    fileUploadInstance.getProgress.bind(fileUploadInstance) as express.RequestHandler
  );
  app.post(
    '/upload/subscribe/:id',
    fileUploadInstance.subscribeToProgress.bind(fileUploadInstance) as express.RequestHandler
  );
  ...
  app.get(
    ...
    hasStoreAccess as any,
    async (req: Request, res: Response) => {
      try {
        const storeId = req.query.storeId
          ? parseInt(req.query.storeId as string)
          : (req.session as any).storeId || 0;
  ...
  app.get(
    '/api/loyalty/member/:id/activity',
    isAuthenticated as any,
    async (req: Request, res: Response) => {
      try {
        const memberId = parseInt(req.params.id);
  ...
  app.post('/api/loyalty/enroll', isAuthenticated as any, async (req: Request, res: Response) => {
    try {
      ...
      const member = await loyaltyServiceInstance.createLoyaltyMember({
        customerId,
        storeId: (customer as any).storeId,
  ...
  app.post('/api/loyalty/redeem', isAuthenticated as any, async (req: Request, res: Response) => {
    try {
      ...
      if (!memberId || !rewardId || !transactionId) {
        return res
          .status(400)
          .json({ message: 'Member ID, reward ID, and transaction ID are required' });
      }
  ...
  app.post(`${apiPrefix}/auth/register`, async (req: Request, res: Response) => {
    ...
    try {
      ...
      if (error instanceof z.ZodError) {
        return res
          .status(400)
          .json({ authenticated: false, message: 'Validation error', errors: error.errors });
      }
  ...
  if (process.env.NODE_ENV !== 'production') {
    app.get(`${apiPrefix}/test/email`, async (req: Request, res: Response) => {
      try {
        ...
        if (!isConnected)
          return res.status(500).json({ success: false, message: 'Email connection failed.' });
        ...
        if (result)
          return res
            .status(200)
            .json({ success: true, message: `Test email sent to ${process.env.EMAIL_USER}` });
        return res.status(500).json({ success: false, message: 'Failed to send test email' });
      } catch (error: unknown) {
        console.error('Test email error:', error);
        return res
          .status(500)
          .json({ success: false, message: `Error: ${(error as Error).message}` });
      }
    });
  }
  ...
  app.get(
    `${apiPrefix}/dashboard/quick-stats`,
    isAuthenticated as any,
    async (req: Request, res: Response) => {
      try {
        ...
        const todaySalesTotal = todaySalesData.reduce(
          (sum, data) => sum + Number(data.totalSales),
          0
        );
        const todayTransactions = todaySalesData.reduce(
          (sum, data) => sum + Number(data.transactionCount),
          0
        );

        const yesterdaySalesData = await storage.getDailySalesData(storeId, 2);
        const yesterdaySalesFiltered = yesterdaySalesData.filter(
          data => data.date && new Date(data.date).getDate() === yesterday.getDate()
        );
        const yesterdaySalesTotal = yesterdaySalesFiltered.reduce(
          (sum, data) => sum + Number(data.totalSales),
          0
        );
        const yesterdayTransactions = yesterdaySalesFiltered.reduce(
          (sum, data) => sum + Number(data.transactionCount),
          0
        );
        ...
        const salesPercentChange =
          yesterdaySalesTotal > 0
            ...
            : todaySalesTotal > 0
              ? 100
              : 0;
        const transactionsPercentChange =
  ...
  app.get(
    `${apiPrefix}/dashboard/store-performance`,
    isAuthenticated as any,
    async (req: Request, res: Response) => {
      try {
        ...
        if (userRole !== 'admin' && storeId) {
          ...
          const singleStorePerformance = [
            {
              ...
              totalSales: storeData
                .reduce((sum, day) => sum + parseFloat(String(day.totalSales)), 0)
                .toFixed(2),
              transactionCount: storeData.reduce(
                (sum, day) => sum + (Number(day.transactionCount) || 0),
                0
              ),
            },
          ];
          return res
            .status(200)
            .json({ storeComparison: singleStorePerformance, dailySales: storeData });
        }
  ...
  app.get(
    `${apiPrefix}/analytics/store-performance`,
    isAuthenticated as any,
    async (req: Request, res: Response) => {
      try {
        ...
        const storePerformanceData = await Promise.all(
          filteredStores.map(async store => {
            ...
            const totalRevenue = transactions.reduce(
              (sum: number, t: any) => sum + parseFloat(t.total.toString()),
              0
            );
            const transactionCount = transactions.length;
            const averageTransaction = transactionCount > 0 ? totalRevenue / transactionCount : 0;
            const topProductsRaw = await (storage as any).getSalesTrends(
              store.id,
              startDate,
              endDate
            );
            ...
            return {
              ...store,
              metrics: { totalRevenue, averageTransaction, transactionCount },
              topProducts,
            };
          })
        );

        const globalMetrics = storePerformanceData.reduce(
          (acc, store) => {
        ...
        return res
          .status(200)
          .json({ storePerformance: storePerformanceData, globalMetrics, dateRangeDescription });
      } catch (error: unknown) {
  ...
  app.get(
    `${apiPrefix}/analytics/sales-trends`,
    isAuthenticated as any,
    async (req: Request, res: Response) => {
      try {
        const { storeId: sessionStoreId, userRole } = req.session as any;
        ...
        let targetStoreId =
          userRole !== 'admin'
            ? sessionStoreId
            : queryStoreId
              ? parseInt(queryStoreId as string)
              : undefined;
        ...
        const dateRangeDescription = analyticsService.getDateRangeDescription(
          parsedStartDate,
          parsedEndDate
        );
  ...
  app.get(`${apiPrefix}/users`, isManagerOrAdmin as any, async (req: Request, res: Response) => {
    try {
      ...
      else if (userRole === 'manager' && sessionStoreId)
        users = await storage.getUsersByStoreId(sessionStoreId);
  ...
  app.post(`${apiPrefix}/users`, isManagerOrAdmin as any, async (req: Request, res: Response) => {
    try {
      ...
      if (sessionUserRole === 'manager') {
        ...
        if (userData.storeId !== sessionStoreId)
          return res
            .status(403)
            .json({ message: 'Managers can only create users for their own store' });
      }
  ...
  app.get(`${apiPrefix}/inventory`, isAuthenticated as any, async (req: Request, res: Response) => {
    try {
      ...
      const requestedStoreId = req.query.storeId
        ...
        : undefined;
      const targetStoreId =
        userRole === 'admin' && requestedStoreId ? requestedStoreId : sessionStoreId;
  ...
  app.put(
    `${apiPrefix}/inventory/:inventoryId`,
    isManagerOrAdmin as any,
    async (req: Request, res: Response) => {
      try {
  ...
  app.post(
    `${apiPrefix}/inventory/minimum-level`,
    isManagerOrAdmin as any,
    async (req: Request, res: Response) => {
      try {
    ...
    async (req: Request, res: Response) => {
      try {
        ...
        if (products.length > 0 && parseInt(products[0].count.toString()) > 0)
          return res
            .status(409)
            .json({ message: 'Cannot delete category used by products', count: products[0].count });
  ...
  app.get(
    `${apiPrefix}/products/barcode/:barcode`,
    isAuthenticated as any,
    async (req: Request, res: Response) => {
      try {
        ...
        const today = new Date();
        if ((inventory as any).expiryDate && new Date((inventory as any).expiryDate) < today)
      ...
      try {
        ...
        const productsWithInventory = await Promise.all(
          products.map(async product => {
            ...
            const isExpired =
              (inventory as any).expiryDate && new Date((inventory as any).expiryDate) < today;
            return {
              ...product,
              inStock: (inventory as any).quantity > 0,
              quantity: (inventory as any).quantity,
              expiryDate: (inventory as any).expiryDate,
              isExpired,
            };
  ...
  // ----------- POS Routes -----------
  app.post(
    `${apiPrefix}/pos/cashier-sessions/start`,
    isAuthenticated as any,
    async (req: Request, res: Response) => {
      try {
        ...
        if (activeSession)
          return res
            .status(400)
            .json({ message: 'You already have an active session', session: activeSession });
  ...
  app.post(
    `${apiPrefix}/pos/cashier-sessions/end`,
    isAuthenticated as any,
    async (req: Request, res: Response) => {
      try {
        const userId = (req.session as any).userId as number;
  ...
  app.get(
    `${apiPrefix}/pos/cashier-sessions/active`,
    isAuthenticated as any,
    async (req: Request, res: Response) => {
      try {
        const userId = (req.session as any).userId as number;
  ...
  app.get(
    `${apiPrefix}/pos/cashier-sessions/:id`,
    isAuthenticated as any,
    async (req: Request, res: Response) => {
      try {
        const userId = (req.session as any).userId as number;
        ...
        if (session.userId !== userId && userRole !== 'admin' && userRole !== 'manager')
          return res
            .status(403)
            .json({ message: "You don't have permission to view this session" });
        return res.status(200).json(session);
      } catch (error: unknown) {
  ...
  app.get(
    `${apiPrefix}/pos/cashier-sessions`,
    isAuthenticated as any,
    async (req: Request, res: Response) => {
      try {
        const userId = (req.session as any).userId as number;
    ...
    async (req: Request, res: Response) => {
      try {
        ...
        if (!activeSession && !transactionData.isOfflineTransaction)
          return res.status(400).json({ message: 'No active cashier session found.' });

        let loyaltyMemberId = null;
        if (transactionData.loyaltyId) {
          const loyaltyMember = await storage.getLoyaltyMemberByLoyaltyId(
            transactionData.loyaltyId
          );
        ...
        const validatedTransaction = schema.transactionInsertSchema.parse({
          ...
          cashierId: userId,
          loyaltyMemberId,
          total: transactionData.totalAmount.toString(),
        }); // Added total
        ...
        for (const item of items) {
          ...
          if (!product.barcode) {
            validationErrors.push(`Product ${product.name} no barcode`);
            continue;
          }
          if (parseFloat(product.price.toString()) <= 0) {
            validationErrors.push(`Product ${product.name} no price`);
            continue;
          }
          const inventory = await storage.getStoreProductInventory(storeId, item.productId);
          if (!inventory) {
            validationErrors.push(`Product ${product.name} not in inventory`);
            continue;
          }
          if ((inventory as any).quantity < item.quantity) {
        ...
        if (validationErrors.length > 0)
          return res
            .status(400)
            .json({ message: 'Product validation failed', errors: validationErrors });
  ...
  app.post(
    `${apiPrefix}/pos/sync-offline-transactions`,
    isAuthenticated as any,
    async (req: Request, res: Response) => {
      try {
        ...
        for (const transaction of transactions) {
          ...
          try {
            if (transaction.transactionData.loyaltyId) {
              const loyaltyMember = await storage.getLoyaltyMemberByLoyaltyId(
                transaction.transactionData.loyaltyId
              );
  ...
  app.get(
    `${apiPrefix}/affiliates/dashboard`,
    isAuthenticated as any,
    async (req: Request, res: Response) => {
      try {
        const userId = (req.session as any).userId as number;
  ...
  app.get(
    `${apiPrefix}/affiliates/referrals`,
    isAuthenticated as any,
    async (req: Request, res: Response) => {
      try {
        const userId = (req.session as any).userId as number;
  ...
  app.post(
    `${apiPrefix}/affiliates/bank-details`,
    isAuthenticated as any,
    async (req: Request, res: Response) => {
      try {
        const userId = (req.session as any).userId as number;
  ...
  app.post(
    ...
    async (req: Request, res: Response) => {
      try {
        ...
        return res
          .status(500)
          .json({ message: (error as Error).message || 'Failed to initialize payment' });
      }
    }
  );

  app.get(
    `${apiPrefix}/payments/verify/:reference/:provider`,
    isAuthenticated as any,
    async (req: Request, res: Response) => {
      try {
        const userId = (req.session as any).userId as number;
        ...
        if (provider === 'simulation') {
          ...
          if (status === 'success') {
            ...
            return res
              .status(200)
              .json({ success: true, subscription, message: 'Payment simulation successful' });
          }
          return res
            .status(200)
            .json({ success: false, status: 'failed', message: 'Payment simulation failed' });
        }
        ...
        return res.status(200).json({
          ...
          message: verification.status === 'pending' ? 'Payment processing' : 'Payment failed',
        });
      } catch (error: unknown) {
        console.error('Payment verification error:', error);
        return res
          .status(500)
          .json({ message: (error as Error).message || 'Failed to verify payment' });
      }
    }
  );

  app.get(
    `${apiPrefix}/subscriptions/current`,
    isAuthenticated as any,
    async (req: Request, res: Response) => {
      try {
        const userId = (req.session as any).userId as number;
  ...
  app.post(
    `${apiPrefix}/subscriptions/process-payout`,
    isAdmin as any,
    async (req: Request, res: Response) => {
      try {
  ...
  app.get(
    `${apiPrefix}/ai/conversation`,
    isAuthenticated as any,
    async (req: Request, res: Response) => {
      try {
        const userId = (req.session as any).userId as number;
        if (!userId) return res.status(401).json({ message: 'Unauthorized' });
        const conversation = (await (storage as any).getAiConversation(userId)) as {
          messages: Array<{ role: string; content: string }>;
        };
        if (!conversation) {
          const welcomeMessage =
            'Hello! Welcome to ChainSync AI Assistant. How can I assist you today?';
          await (storage as any).saveAiConversation(userId, [
            { role: 'assistant', content: welcomeMessage },
          ]);
          return res
            .status(200)
            .json({ messages: [{ role: 'assistant', content: welcomeMessage }] });
        }
  ...
  app.post(`${apiPrefix}/returns/reasons`, isAdmin as any, async (req: Request, res: Response) => {
    try {
      ...
      const reasonData = (schema as any).returnReasonInsertSchema.parse({
        name,
        description,
        active: true,
      });
    ...
    async (req: Request, res: Response) => {
      try {
        ...
        if (
          ...
          (req.session as any).userRole !== 'manager'
        )
          storeId = (req.session as any).storeId?.toString();
        if (
          (req.session as any).userRole === 'manager' &&
          storeId &&
          (req.session as any).storeId !== parseInt(storeId as string)
        )
    ...
    async (req: Request, res: Response) => {
      try {
        ...
        if (!isNaN(parseInt(returnId)))
          returnData = await (storage as any).getReturnById(parseInt(returnId));
        ...
        if (
          (req.session as any).userRole !== 'admin' &&
          returnData.storeId !== (req.session as any).storeId
        )
  ...
  app.post(`${apiPrefix}/returns`, isAuthenticated as any, async (req: Request, res: Response) => {
    try {
      const { originalTransactionId, storeId, customerId, totalRefundAmount, items, notes } =
        req.body;
      if ((req.session as any).userRole !== 'admin' && (req.session as any).storeId !== storeId)
        return res.status(403).json({ error: 'You do not have access to this store' });
      const returnId = `RET-${Date.now()}-${Math.floor(Math.random() * 1000)}`;
      const returnData = (schema as any).returnsInsertSchema.parse({
        returnId,
      ...
      const returnItems = (items as any[]).map((item: any) =>
        (schema as any).returnItemsInsertSchema.parse({
  ...
  app.patch(
    `${apiPrefix}/returns/:returnId/status`,
    isManagerOrAdmin as any,
    async (req: Request, res: Response) => {
      try {
        const { returnId } = req.params;
        ...
        if (
          ...
          return res.status(403).json({ error: 'You do not have access to this return' });
        const updatedReturn = await (storage as any).updateReturnStatus(returnData.id, status);
  ...
  app.get(
    ...
    async (req: Request, res: Response) => {
      try {
        ...
        if (
          (req.session as any).userRole !== 'admin' &&
          customer.storeId !== (req.session as any).storeId
        )
  ...
  app.post(
    ...
    async (req: Request, res: Response) => {
      try {
        ...
        const customerData = (schema as any).customersInsertSchema.parse({
          fullName,
          email,
          phone,
          storeId,
  ...
  app.post(
    `${apiPrefix}/inventory/batches/:batchId/adjust`,
    isAuthenticated as any,
    isManagerOrAdmin as any,
    async (req: Request, res: Response) => {
      try {
        ...
        return res
          .status(500)
          .json({ message: (error as Error).message || 'Failed to adjust batch stock' });
      }
    }
  );

  app.delete(
    `${apiPrefix}/inventory/batches/:batchId`,
    isAuthenticated as any,
    isManagerOrAdmin as any,
    async (req: Request, res: Response) => {
      try {
        ...
        if (currentBatch.quantity > 0 && !forceDelete)
          return res.status(400).json({
            message: 'Cannot delete batch with non-zero quantity.',
        ...
        return res
          .status(500)
          .json({ message: (error as Error).message || 'Failed to delete batch' });
      }
    }
  );

  app.get(
    `${apiPrefix}/inventory/batches/:batchId/audit-logs`,
    isAuthenticated as any,
    isManagerOrAdmin as any,
    async (req: Request, res: Response) => {
      try {
