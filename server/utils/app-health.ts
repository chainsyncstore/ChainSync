...
export class AppHealthManager extends EventEmitter {
   ...
   */
  public registerComponent(name: string, checkFn: () => Promise<ComponentHealth>): void {
  ...
  private updateMetrics(health: AppHealth): void {
    ...
    if (dbComponent && dbComponent.details) {
      metricsCollector.setCustomMetric(
        'db_response_time_ms',
        dbComponent.details.responseTime || 0
      );
      metricsCollector.setCustomMetric(
        'db_active_connections',
        dbComponent.details.activeConnections || 0
      );
      metricsCollector.setCustomMetric(
        'db_idle_connections',
        dbComponent.details.idleConnections || 0
      );
      metricsCollector.setCustomMetric(
        'db_waiting_clients',
        dbComponent.details.waitingClients || 0
      );
    }
    ...
    for (const component of health.components) {
      metricsCollector.setCustomMetric(
        `health_${component.name}`,
        this.statusToNumeric(component.status)
      );
    }
  }
