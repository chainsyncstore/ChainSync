...
export const up: Migration['up'] = async () => {
  ...
  await db.execute(sql`CREATE INDEX IF NOT EXISTS idx_order_items_product_id ON order_items(product_id)`);
  
  // Loyalty Programs
  await db.execute(sql`CREATE INDEX IF NOT EXISTS idx_loyalty_programs_store_id ON loyalty_programs(store_id)`);
  await db.execute(sql`CREATE INDEX IF NOT EXISTS idx_loyalty_programs_is_active ON loyalty_programs(is_active)`);
  
  // Loyalty Tiers
  await db.execute(sql`CREATE INDEX IF NOT EXISTS idx_loyalty_tiers_program_id ON loyalty_tiers(program_id)`);
  
  // Loyalty Members
  await db.execute(sql`CREATE INDEX IF NOT EXISTS idx_loyalty_members_program_id ON loyalty_members(program_id)`);
  await db.execute(sql`CREATE INDEX IF NOT EXISTS idx_loyalty_members_customer_id ON loyalty_members(customer_id)`);
  await db.execute(sql`CREATE INDEX IF NOT EXISTS idx_loyalty_members_tier_id ON loyalty_members(tier_id)`);
  await db.execute(sql`CREATE INDEX IF NOT EXISTS idx_loyalty_members_loyalty_id ON loyalty_members(loyalty_id)`);
  await db.execute(sql`CREATE INDEX IF NOT EXISTS idx_loyalty_members_is_active ON loyalty_members(is_active)`);
  
  // Loyalty Rewards
  await db.execute(sql`CREATE INDEX IF NOT EXISTS idx_loyalty_rewards_program_id ON loyalty_rewards(program_id)`);
  await db.execute(sql`CREATE INDEX IF NOT EXISTS idx_loyalty_rewards_is_active ON loyalty_rewards(is_active)`);
  
  // Loyalty Transactions
  await db.execute(sql`CREATE INDEX IF NOT EXISTS idx_loyalty_transactions_member_id ON loyalty_transactions(member_id)`);
  await db.execute(sql`CREATE INDEX IF NOT EXISTS idx_loyalty_transactions_program_id ON loyalty_transactions(program_id)`);
  await db.execute(sql`CREATE INDEX IF NOT EXISTS idx_loyalty_transactions_transaction_id ON loyalty_transactions(transaction_id)`);
  await db.execute(sql`CREATE INDEX IF NOT EXISTS idx_loyalty_transactions_reward_id ON loyalty_transactions(reward_id)`);
  await db.execute(sql`CREATE INDEX IF NOT EXISTS idx_loyalty_transactions_created_at ON loyalty_transactions(created_at)`);
  await db.execute(sql`CREATE INDEX IF NOT EXISTS idx_loyalty_transactions_type ON loyalty_transactions(type)`);
};
