...
class RedisRateLimitStore implements RateLimitStore {
  ...
  async increment(key: string, windowMs: number): Promise<{ count: number; resetTime: number }> {
    ...
    try {
      ...
      logger.error(
        'Redis rate limit increment error:',
...
export function createRateLimit(options: RateLimitOptions = {}) {
  ...
  return async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      logger.error(
        'Rate limiting middleware encountered an error',
        error instanceof Error ? error : new Error(String(error))
      );
...
export function createAdaptiveRateLimit(
  ...
  anonymousOptions: RateLimitOptions
) {
...
export function createProgressiveRateLimit(baseOptions: RateLimitOptions) {
  ...
  logger.warn(
    'Progressive rate limiting with dynamic max based on trust level is not fully implemented by default createRateLimit. Key will include trust level.'
  );
...
function getUserTrustLevel(req: Request): 'low' | 'medium' | 'high' {
  ...
  try {
    ...
    const createdAtTime = new Date(user.createdAt).getTime(); // Cast user.createdAt
    if (isNaN(createdAtTime)) {
      logger.warn('Invalid createdAt date for user trust level calculation', {
        userId: user.id,
    ...
    logger.error(
      'Error calculating user trust level from createdAt',
...
export function createBurstRateLimit(
  shortTermOptions: RateLimitOptions,
  longTermOptions: RateLimitOptions
) {
