...
class AuthenticationService {
  ...
  constructor() {
    ...
    if (!this.redis) {
      ...
      if (!this.redis) {
        logger.warn(
          'Redis client not available. Using fallback in-memory storage. This is NOT recommended for production!'
        );
      } else {
  ...
  async generateTokenPair(
    ...
    try {
      ...
      if (this.redis) {
        ...
        await this.redis.set(`${this.TOKEN_PREFIX}${sessionId}`, refreshToken, 'EX', expirySeconds);
...
export const requireStoreAccess = (store_idParam = 'store_id') => {
  return (req: Request, res: Response, next: NextFunction) => {
    ...
    if ((req.user as UserPayload).storeId !== store_id) {
      // Cast req.user
