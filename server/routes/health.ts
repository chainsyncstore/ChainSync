...
async function checkDatabase(): Promise<{
  ...
  try {
    ...
    await dbManager.executeQuery(db => db.execute('SELECT 1'), 'health-check');
  ...
  try {
    ...
    for (const queueType of Object.values(QueueType)) {
      try {
        ...
        logger.warn(
          `Failed to get stats for queue ${queueType}`,
          error instanceof Error ? error : new Error(String(error))
        );
  ...
  try {
    ...
    if (queueResult.queues) {
      for (const [queueType, stats] of Object.entries(queueResult.queues)) {
        metrics.push(
          `# HELP queue_${queueType}_waiting_jobs Number of waiting jobs in ${queueType} queue`
        );
        ...
        metrics.push(
          `# HELP queue_${queueType}_active_jobs Number of active jobs in ${queueType} queue`
        );
        metrics.push(`# TYPE queue_${queueType}_active_jobs gauge`);
        metrics.push(`queue_${queueType}_active_jobs ${stats.active} ${timestamp}`);

        metrics.push(
          `# HELP queue_${queueType}_delayed_jobs Number of delayed jobs in ${queueType} queue`
        );
        ...
        metrics.push(
          `# HELP queue_${queueType}_failed_jobs Number of failed jobs in ${queueType} queue`
        );
    ...
    try {
      ...
      if (redisClient) {
        ...
        metrics.push(
          `redis_response_time_milliseconds ${redisResponseTime.toFixed(2)} ${timestamp}`
        );
