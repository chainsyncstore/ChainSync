...
router.get('/', async (req: Request, res: Response, next: NextFunction) => {
  ...
  try {
    ...
    const conditions = [];
    if (customerId)
      conditions.push(eq(schema.transactions.customerId, parseInt(customerId as string)));
...
router.get('/:id', async (req: Request, res: Response, next: NextFunction) => {
  ...
  try {
    ...
    if (!transaction) {
      return res
        .status(404)
        .json({ error: 'Transaction not found', code: 'TRANSACTION_NOT_FOUND' });
    }

    const loyaltyUpdates = await db
      .select()
      .from((schema as any).loyaltyUpdates) // Assuming loyaltyUpdates schema exists
      .where(eq((schema as any).loyaltyUpdates.transactionId, transactionIdNum));
  ...
  async (req: Request, res: Response, next: NextFunction) => {
    ...
    try {
      ...
      const transactionDbData = {
        ...
        paymentStatus,
        referenceNumber:
          referenceNumber || `TXN-${Date.now()}-${Math.random().toString(36).substring(2, 7)}`,
        createdAt: new Date(),
        updatedAt: new Date(),
      };
      ...
      if (items && Array.isArray(items) && items.length > 0) {
        const transactionItemsToInsert = items.map((item: any) => {
          // item is from createTransactionBodySchema.items
          const itemData = {
            transactionId: transaction.transactionId,
      ...
      if (type === 'purchase') {
        ...
        reqLogger.info('Loyalty processing queued', {
          transactionId: transaction.transactionId,
          customerId,
        });
      }
