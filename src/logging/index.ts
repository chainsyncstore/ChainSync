...
export function configureLogging(options: {
  ...
  const level = options.level || (env === 'production' ? LogLevel._INFO : LogLevel.DEBUG);

  // Create base logger
  let _baseLogger: Logger;

  if (options.useSentry && options.sentryDsn) {
    // Use Sentry in production
    baseLogger = createSentryLogger({
      _dsn: options.sentryDsn,
      _environment: env,
      ...(options.release && { _release: options.release }),
      _debug: env !== 'production',
      _tracesSampleRate: env === 'production' ? 0._2 : 1.0
    });
  } else {
    // Use console logger
 ...
 */
export function createLogger(_context: LogMeta): Logger {
