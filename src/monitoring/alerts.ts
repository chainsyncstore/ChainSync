...
export function setupAlerts(
  ...
  dbPool: { idleCount: number; totalCount: number; [key: string]: any } | any
): void {
  // Using 'any' as a broad fallback for dbPool
    ...
    if (dbPool) {
      ...
      if (usedPercent >= ALERT_THRESHOLDS.DB_CONNECTIONS_CRITICAL) {
        triggerAlert(
          ...
          `Database connections critical: ${usedPercent}%`,
          'critical'
        );
      } else if (usedPercent >= ALERT_THRESHOLDS.DB_CONNECTIONS_WARNING) {
        triggerAlert(
          'db_connections_warning',
          `Database connections warning: ${usedPercent}%`,
          'warning'
        );
      }
    }
