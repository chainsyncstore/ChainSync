...
export function configureAlertThresholds() {
  ...
  if (process.env.ALERT_HEALTH_CHECK_FAILURE_COUNT) {
    thresholds.HEALTH_CHECK_FAILURE_COUNT = parseInt(
      process.env.ALERT_HEALTH_CHECK_FAILURE_COUNT,
      10
    );
  }
...
export function httpMetricsMiddleware(req: Request, res: Response, next: NextFunction) {
  ...
  res.end = function (...args: unknown[]) {
    ...
    httpRequestDurationMicroseconds.labels(method, route, statusCode).observe(responseTime);

    // Increment request counter
    httpRequestsTotalCounter.labels(method, route, statusCode).inc();

    // Track errors (5xx)
    if (statusCode.startsWith('5')) {
      httpRequestsErrorCounter.labels(method, route, statusCode, 'server_error').inc();
      ...
      alertCounterGauge.labels('http_5xx_rate', 'warning').set(1);
    }
...
export function trackMemoryUsage() {
  ...
  if (memoryPercent > thresholds.HIGH_MEMORY_PERCENT) {
    alertCounterGauge.labels('high_memory_usage', 'critical').set(1);

    logger.warn(
      `High memory usage alert: ${memoryPercent}% (threshold: ${thresholds.HIGH_MEMORY_PERCENT}%)`,
      {
        memoryPercent,
        rss: memUsage.rss,
    ...
    alertCounterGauge.labels('high_memory_usage', 'critical').set(0);
  }
}
...
export function trackDbQuery(queryType: string, durationMs: number, error?: Error) {
  ...
  dbQueryDurationMicroseconds.labels(queryType).observe(durationMs);

  // Check for slow query alert
  const thresholds = configureAlertThresholds();
  if (durationMs > thresholds.SLOW_DB_RESPONSE_MS) {
    alertCounterGauge.labels('slow_db_response', 'warning').set(1);

    logger.warn(
      `Slow DB query alert: ${durationMs}ms (threshold: ${thresholds.SLOW_DB_RESPONSE_MS}ms)`,
      {
        queryType,
        durationMs,
      }
  ...
  if (error) {
    dbErrorsCounter.labels(queryType, error.name || 'unknown').inc();
...
export function trackHealthCheck(
  ...
  healthCheckStatusGauge.labels(checkType, component).set(isUp ? 1 : 0);

  // Record health check duration
  healthCheckDurationMicroseconds.labels(checkType, component).observe(durationMs);

  // Check for health check failures
  if (!isUp) {
    alertCounterGauge.labels(`${component}_health_check_failure`, 'critical').set(1);
    ...
    alertCounterGauge.labels(`${component}_health_check_failure`, 'critical').set(0);
  }
}
 ...
 * Track queue metrics
 */
export function trackQueueMetrics(
  queueName: string,
  waiting: number,
  active: number,
  delayed: number,
  ...
  if (waiting > 100 || failed > 10) {
    alertCounterGauge.labels(`${queueName}_queue_backlog`, 'warning').set(1);
    ...
    alertCounterGauge.labels(`${queueName}_queue_backlog`, 'warning').set(0);
  }
}
