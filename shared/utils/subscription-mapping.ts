...
import type { Subscription } from '@server/services/subscription/types'; // Corrected import path
 ...
 */
export function subscriptionToDatabaseFields(subscription: Partial<Subscription>): Record<string, unknown> {
 ...
 */
export function subscriptionFromDatabaseFields<T extends Record<string, unknown>>(dbSubscription: T): Partial<Subscription> {
  ...
  try {
    ...
    return validateEntity(subscriptionSchema.partial(), camelCaseObject, 'database subscription object');
  } catch (error) {
    ...
    console.error(
      "Field mapping issue: Mapped object from database does not conform to Partial<Subscription> structure.",
      {
        ...
        validationErrorDetails: error instanceof SchemaValidationError 
          ? error.toJSON() 
    ...
    throw new Error(`Failed to map subscription from database due to validation error: ${String(error)}`);
  }
}
 ...
 */
export function mapSubscriptionArray<T extends Record<string, unknown>>(dbSubscriptions: T[]): Partial<Subscription>[] {
