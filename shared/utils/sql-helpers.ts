 ...
 */
export function formatDateForSql(_date: Date | string | null | undefined): string {
  if (!date) return 'NULL';
 ...
 */
export function formatJsonForSql(_data: any): string {
  if (!data) return 'NULL';
...
export function buildRawInsertQuery(
  _tableName: string,
  _data: Record<string, string>, // Values should already be formatted as SQL-safe strings
  _returnFields: string[] = ['*']
): string {
 ...
 */
export function prepareSqlValues(_data: Record<string, any>): Record<string, string> {
