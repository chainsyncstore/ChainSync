...
export const users = pgTable(
  'users',
  {
    ...
    storeId: integer('store_id').references(
      /* Will be set in relations */ () => (global as any).stores.id,
      { onDelete: 'set null' }
    ), // Assuming global.stores.id
...
export const userInsertSchema = createInsertSchema(users, {
  ...
  password: schema => schema.min(8, 'Password must be at least 8 characters'), // Input validation, hashing handled separately
  fullName: schema => schema.min(1, 'Full name is required'),
  email: schema => schema.email('Invalid email'),
  role: schema => schema.optional(), // DB has default
  storeId: schema => schema.int().positive().optional().nullable(), // Nullable FK
...
export const passwordResetTokenInsertSchema = createInsertSchema(passwordResetTokens, {
  userId: schema => schema.int().positive(), // Non-null FK
  token: schema => schema.min(32, 'Token must be at least 32 characters'),
  expiresAt: schema => schema, // Let drizzle infer or customize if needed
  used: schema => schema.optional(), // DB has default
});
