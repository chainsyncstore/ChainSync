...
export const transactionInsertSchema = baseInsertSchema.extend({
  ...
  referenceNumber: z
    ...
    .optional(), // Made optional as it might be auto-generated
});
...
export const transactionItemInsertSchema = baseInsertSchema.extend({
  ...
  unitPrice: z
    ...
    .max(100000, 'Unit price cannot exceed 100,000'), // Zod expects number, Drizzle handles decimal conversion
  discount: z
    .number()
...
export const transactionRelations = relations(transactions, helpers => ({
  ...
  user: helpers.one(users, {
    // This is the general user who might have initiated (e.g. online order)
    fields: [transactions.userId],
    references: [users.id],
  }),
  cashier: helpers.one(users, {
    // This is the cashier who processed at POS
    fields: [transactions.cashierId],
    references: [users.id],
  }),
  customer: helpers.one(users, {
    // This is the customer associated with the transaction
