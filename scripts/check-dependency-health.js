...
function checkOutdatedDependencies() {
  ...
  try {
    ...
    console.log(`${colors.bold}Package Name${' '.repeat(20)}Current${' '.repeat(10)}Latest${' '.repeat(10)}Type${colors.reset}`);
...
function checkSecurityVulnerabilities() {
  ...
  try {
    ...
    const auditOutput = execSync('npm audit --json', { encoding: 'utf8', cwd: rootDir, stdio: ['pipe', 'pipe', 'pipe'] });
      ...
      if (criticalVulns.length > 0) {
        ...
        criticalVulns.forEach(vuln => {
          ...
          if (vuln.fixAvailable) {
            console.log(`  ${colors.green}Fix available: ${typeof vuln.fixAvailable === 'object' ? 
    ...
    console.log(`Run ${colors.bold}npm audit fix${colors.reset} to automatically fix vulnerabilities that have available patches.`);
    console.log(`For more details, run ${colors.bold}npm audit${colors.reset} to see the full report.`);
    
    return false;
  } catch (error) {
    if (error.message.includes('Command failed') && error.message.includes('vulnerabilities found')) {
...
function checkLicenseCompliance() {
  ...
  try {
    ...
    const problematicDeps = uniqueDependencies.filter(dep => 
      dep.license && restrictiveLicenses.some(license => 
        typeof dep.license === 'string' && dep.license.includes(license)
      )
    );
    ...
    console.log(`${colors.yellow}! Found ${problematicDeps.length} dependencies with potentially problematic licenses:${colors.reset}\n`);
    
    // Create a table to display problematic packages
    console.log(`${colors.bold}Package Name${' '.repeat(30)}Version${' '.repeat(15)}License${colors.reset}`);
    console.log('-'.repeat(75));
  ...
  - Consulting with legal counsel`);
    ...
    console.error(`${colors.red}Error checking license compliance: ${error.message}${colors.reset}`);
    return false;
  }
}
...
function runAllChecks() {
  ...
  console.log(`Outdated Dependencies: ${results.outdated ? `${colors.green}✓ OK${colors.reset}` : `${colors.yellow}⚠ Needs attention${colors.reset}`}`);
  console.log(`Security Vulnerabilities: ${results.security ? `${colors.green}✓ OK${colors.reset}` : `${colors.red}✕ Critical issues${colors.reset}`}`);
  console.log(`License Compliance: ${results.license ? `${colors.green}✓ OK${colors.reset}` : `${colors.yellow}⚠ Needs review${colors.reset}`}`);
