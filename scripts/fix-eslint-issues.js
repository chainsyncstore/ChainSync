...
const eslintCommands = {
  ...
  imports:
    'eslint . --ext .ts,.tsx --rule "import/no-duplicates: error" --rule "import/order: error" --quiet',
  hooks:
    'eslint ./client/src --ext .tsx --rule "react-hooks/rules-of-hooks: error" --rule "react-hooks/exhaustive-deps: error" --quiet',
    ...
    if (totalIssues > 0) {
      ...
      for (const category in serverResults.issuesByCategory) {
        combinedCategories[category] = {
          total:
            serverResults.issuesByCategory[category].total +
